...
export function GlassCard({ children, className = "", style = {}, hover = true, fxStatus = "linked", isDark = true }: {
  ...
  function onMove(e: React.MouseEvent<HTMLDivElement>) {
    ...
    const rx = (0.5 - py) * 22;
    const ry = (px - 0.5) * 22;
    ref.current.style.transform = `perspective(900px) rotateX(${rx.toFixed(2)}deg) rotateY(${ry.toFixed(2)}deg) translateY(-6px) scale(1.02)`;
    ref.current.style.borderColor = `rgba(${TEAL},0.55)`;
    ref.current.style.boxShadow = `0 0 70px rgba(${TEAL},0.25), inset 0 1px 0 rgba(255,255,255,0.1), 0 26px 70px rgba(0,0,0,0.55)`;
    glareRef.current.style.background = `radial-gradient(circle at ${(px * 100).toFixed(1)}% ${(py * 100).toFixed(1)}%, rgba(${TEAL},0.28), transparent 55%)`;
